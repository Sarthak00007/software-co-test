...
const SelectColumnsModal = ({ isOpen, onClose, onApply, columns, selectedColumns }) => {
  const [localSelected, setLocalSelected] = useState(selectedColumns || []);

  const handleToggle = (column) => {
    if (localSelected.includes(column)) {
      setLocalSelected(localSelected.filter(c => c !== column));
    } else {
      setLocalSelected([...localSelected, column]);
    }
  };
        ...
        <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
          <div className="bg-white dark:bg-gray-800 px-6 pt-6 pb-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Select Columns</h3>
              <button
                onClick={onClose}
